...
from ._field_stats import format_console_summary, summarize_results, summary_to_dict
...
def _flatten(all_results: dict[str, list[BenchmarkResult]]) -> list[BenchmarkResult]:
    """Flatten the per-scenario result mapping into a single list."""
...
def _save_results(
    ...
    payload: dict[str, Any] = {
        ...
        "models": MODELS,
        "scenarios": {},
        "field_issue_summary": summary_to_dict(summarize_results(_flatten(all_results))),
    }
...
def _build_report_md(
    ...
    # --- Field issue analysis (cross-run, per schema) ---
    summary = summarize_results(_flatten(all_results))
    if any(s.field_problems for s in summary.schemas):
        lines.append("## Field Issue Analysis")
        lines.append("")
        lines.append(
            "_Which fields failed schema validation during extraction, aggregated "
            "across every model and scenario. Fields are sorted by severity "
            "(failure-causing first) — a low recovery rate marks a field worth "
            "improving the description/prompt for._"
        )
        lines.append("")
        # Demote the standalone report's headings (``#``/``##``) so they nest
        # under this section instead of competing with the report's own H1.
        lines.append(_demote_headings(summary.to_markdown()))
        lines.append("")
...
def _demote_headings(md: str) -> str:
    """Add one ``#`` level to every ATX heading line so it nests under a parent."""
    out: list[str] = []
...
async def run_benchmarks(
    ...
    issue_lines = format_console_summary(summarize_results(_flatten(all_results)))
